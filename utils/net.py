...
if __name__ == "__main__":
    data = cv2.imread(  '../data/VOC2007/JPEGImages/000005.jpg' )
    data = cv2.cvtColor( data, cv2.COLOR_BGR2RGB )
    data = cv2.resize( data, ( 416, 416 ) )

    data = tf.cast( tf.expand_dims( tf.constant( data ), 0 ), tf.float32 )

    pre_scale1, pre_scale2, pre_scale3 = feature_extractor( data, True )

    scale_1, scale_2, scale_3 = scales( pre_scale1, pre_scale2, pre_scale3,True )
